---
title: "Quick Portfolio (PDF)"
excerpt: "A single-page PDF overview of my major VR, AI, and game development projects.<br/><img src='/images/quick_thumbnail.png' style='width:100%; border-radius:12px;' />"
collection: portfolio
permalink: /portfolio/quick-portfolio/
---

# Quick Portfolio

This page contains a **fast, consolidated overview** of my key projects in:
- Virtual Reality (VR)  
- Adaptive & AI-driven systems  
- Game development (2D + 3D)  
- UX/UI and interaction design  

It is ideal for **application reviewers**, **recruiters**, and **professors** who want to see the full breadth of my work at a glance.

---

## 📄 View the PDF Portfolio

- 👉 **[Open PDF in new tab](/files/LINH_PHAM_PORTFOLIO.pdf)**  
- 💾 **[Download PDF](/files/LINH_PHAM_PORTFOLIO.pdf)**  
- 👇 **Embedded preview:** scroll to view

<embed src="/files/LINH_PHAM_PORTFOLIO.pdf" 
       type="application/pdf"
       width="100%" 
       height="900px" 
       style="border-radius: 12px; border: 1px solid #ccc;" />

---

## 🧭 Why This Page Exists

This “Quick Portfolio” is a **single-page snapshot**, giving viewers a fast understanding of:

- what I’ve built  
- what tools I use  
- what problems I solve  
- how my projects connect  
- how my skills have evolved  

For deeper technical breakdowns, viewers can visit the individual project pages through the main **[Portfolio](/portfolio/)** menu.

---
